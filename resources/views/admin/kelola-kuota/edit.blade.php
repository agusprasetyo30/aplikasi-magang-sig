		...
		<div class="col-6">
			<div class="card card-primary">
				<div class="card-header">
					<h5 class="card-title"><i class="fa fa-user-plus mr-2"></i> Edit Kuota Penerimaan KP</h5>
				</div>
				<div class="card-body">
					<form action="{{ route('admin.kelola-kuota.update', $kelola_kuota->id) }}" method="post">
						@csrf
						@method('put')
						...
						<div class="form-group">
							...
							<input type="number" name="kuota" id="kuota" class="form-control" placeholder="Input kuota KP" value="{{ $kelola_kuota->kuota }}">
						</div>
						<div class="form-group">
...
@push('js')
	<script>
		...
		let selected_jurusan = new Option('{!! $kelola_kuota->jurusan->name !!}', '{!! $kelola_kuota->jurusan_id !!}', "selected");
		$('.select2[name="jurusan"]').append(selected_jurusan);

		$('.select2[name=jenjang_pendidikan]').val('{!! $kelola_kuota->jenjang_pendidikan !!}').select2().trigger('change');
		$('.select2[name=bulan_pelaksanaan]').val('{!! $kelola_kuota->bulan_pelaksanaan !!}').select2().trigger('change');
